...
const serviceEnvs = {
  ...
  content: {
    ...
    AWS_CLOUDFRONT_DOMAIN:
      process.env.AWS_CLOUDFRONT_DOMAIN || "your-cloudfront-domain",
    AWS_CLOUDFRONT_PRIVATE_KEY:
      process.env.AWS_CLOUDFRONT_PRIVATE_KEY || "your-cloudfront-private-key",
    AWS_CLOUDFRONT_KEY_PAIR_ID:
      process.env.AWS_CLOUDFRONT_KEY_PAIR_ID || "your-key-pair-id",
    AWS_MEDIACONVERT_ENDPOINT:
      process.env.AWS_MEDIACONVERT_ENDPOINT || "your-mediaconvert-endpoint",
    AWS_MEDIACONVERT_ROLE:
      process.env.AWS_MEDIACONVERT_ROLE || "your-mediaconvert-role-arn",
  },
  streaming: {
...
const commonEnv = {
  ...
  DB_HOST: process.env.DB_HOST || "111.118.251.133",
  DB_PORT: process.env.DB_PORT || "3306",
  DB_USER: process.env.DB_USER || "idealuser",
...
const generateEnvContent = (serviceVars) => {
  ...
  Object.entries(allVars).forEach(([key, value]) => {
    // Special handling for values that might contain special characters
    if (
...
const createServiceEnv = (serviceName, serviceVars) => {
  ...
  try {
    ...
    console.error(
      `❌ Failed to create .env for ${serviceName}:`,
      error.message,
    );
  }
};
...
console.log(
  "2. Each service now has its own .env file with resolved variables",
);
console.log(
  "3. Service-specific variables are configured with actual values or defaults",
);
console.log(
