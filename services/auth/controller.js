...
const authController = {
    ...
    try {
      ...
      if (!user) {
        ...
        if (defaultProfile) {
          ...
          await admin
            .auth()
            .setCustomUserClaims(user.firebaseUid, customClaims);
          logger.info(
            `Custom claims set for user ${user.firebaseUid}:`,
            customClaims
          );
        }
  ...
  async register(req, res) {
    try {
      ...
      const defaultUsername =
        displayName || userEmail || userRecord.displayName;

      try {
        // Create default profile in user service
        const defaultProfile = await authController.createDefaultProfile(
          firebaseUid,
          defaultUsername,
          userEmail
        );
        ...
        logger.info(
          `User registered: ${firebaseUid} with default profile: ${defaultProfile.id}`
        );
      ...
      if (userId) {
        ...
        if (recentLogin) {
          ...
          const sessionDuration = Math.floor(
            (logoutTime - recentLogin.loginAt) / 1000
          ); // in seconds
  ...
  async logoutAll(req, res) {
    try {
      ...
      for (const session of activeSessions) {
        const sessionDuration = Math.floor(
          (logoutTime - session.loginAt) / 1000
        );
  ...
  async switchProfile(req, res) {
    try {
      ...
      try {
        ...
        logger.warn(
          "Could not fetch profile details for child check:",
          dbError.message
        );
      ...
      logger.info(
        `Profile switched for user ${uid} to profile ${profileId}, child: ${isChildProfile}`
      );
  ...
  async createDefaultProfile(firebaseUid, username, userEmail) {
    ...
    try {
      logger.info(
        `Starting profile creation for user: ${firebaseUid} with username: ${username}`
      );
      ...
      logger.error(
        "Error creating default profile - transaction rolled back:",
        {
          error: error.message,
          stack: error.stack,
          firebaseUid,
